...
import { createFixture, directory, session } from "./composer-fixture.mjs"
...
await page.route("**/*", (route) => {
  ...
  if (url.pathname === "/oc/experimental/session") return route.fulfill({ json: [session("ses_a"), { ...session("ses_b"), parentID: "ses_a" }] })
  if (url.pathname === "/oc/session/status" && statusFailed) return route.fulfill({ status: 503, json: {} })
  if (url.pathname === "/oc/session/status" && childBusy) return route.fulfill({ json: { ses_a: { type: "idle" }, ses_b: { type: "busy" } } })
...
try {
  ...
  statusFailed = true
  await page.clock.fastForward(30_000)
  await expect(bar()).toHaveAttribute("data-state", "disconnected")
  await expect(bar().locator(".session-work-spinner")).toHaveCount(0)
  console.log("PASS stale descendant activity becomes unavailable rather than false busy")
